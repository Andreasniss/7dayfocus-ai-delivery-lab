...
- The first exact-head native regression attempts completed the assertions but failed to report a successful run: Android returned `Process crashed` after activity teardown. Android's exit history recorded `EXIT_SELF`, status 0, and the lifecycle log showed the test destroying the last Tauri activity before final reporting. Corrected only the test lifecycle: require the runner's `waitForActivitiesToComplete=false` argument, keep the activity alive through result reporting, and explicitly stop/relaunch afterward. No application lifecycle or assertion was weakened. The exact-head PR record must contain an explicit `OK (1 test)` result before this gate closes.
